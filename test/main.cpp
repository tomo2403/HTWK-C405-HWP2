...
void processIncomingQueue(std::vector<uint8_t> &outputData)
{
    while (true)
    {
        ...
        uint32_t receivedCrc =
                (packet.second[packet.second.size() - 4] << 24) | (packet.second[packet.second.size() - 3] << 16) |
...
void processOutgoingQueue()
{
    ...
    while (true)
    {
        ...
        if (nextPacketId < outgoingData.size())
        {
            ...
            sendDataThread = std::thread([&sendDataPromise](const std::vector<uint8_t> &data) {
                sendData(data);
                sendDataPromise.set_value();
            }, std::ref(packet));

            std::unique_lock<std::mutex> lock(mtx);
            if (cv.wait_for(lock, std::chrono::seconds(10), [] { return !cp.responses.empty(); }))
            {
        ...
        Logger(INFO, true) << "Sending packet " << nextPacketId << " of " << outgoingData.size() << " packets. Errors: "
                           << errors;
    }
}
